...
def getFeatures(dir, filename):
    #Load mp4 file
    capture = cv2.VideoCapture("../Testing-Data/Subset/"+dir+"/"+filename)
    capHeight = capture.get(cv2.CAP_PROP_FRAME_HEIGHT)
    ...
    while (capture.isOpened()):
        ...
        if ret:
            ...
            frameCollection.append([timeStamp,values[0], values[1], values[2], imageHSV, centerHSV, len(frameObjects), frameObjects])
    ...
    csvName = "csvFiles/"+filename[0:len(filename)-4] + ".csv"
    featureFile = open(csvName, 'w')
